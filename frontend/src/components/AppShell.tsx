...
const tools = [
  { icon: BookOpen, label: "Editor", view: "editor" },
  { icon: Layers, label: "Flashcards", view: "flashcards" },
  { icon: CheckCircle2, label: "Quizzes", view: "quiz" },
  { icon: Headphones, label: "Podcast", view: "podcast" },
  { icon: MessageSquare, label: "Assistant", view: "chat" },
];
...
export default function AppShell() {
  ...
  const [currentNoteId, setCurrentNoteId] = useState<string | null>(
    () => sessionStorage.getItem("lastNoteId") ?? null
  );

  // Sync from sessionStorage in case navigation happened outside AppShell
  useEffect(() => {
    const id = sessionStorage.getItem("lastNoteId");
  ...
  // Navigate to the note view for the current note
  function goToTool(view: string) {
    if (currentNoteId) {
      navigate(`/notes/${currentNoteId}/${view}`);
    } else {
      // No note selected - navigate to dashboard
      navigate("/app");
    }
  }
  ...
  async function createBlank() {
    ...
    bump();
    sessionStorage.setItem("lastNoteId", note.id);
        ...
        <nav className="mt-6 space-y-1">
          ...
          {tools.map((t) => (
            ...
            >
              ...
              {t.label}
              {!currentNoteId && <span className="ml-auto text-[10px] text-muted-foreground">Open a note</span>}
            </button>
          ))}
        </nav>
