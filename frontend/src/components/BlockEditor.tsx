...
/* Custom block editor: click a block to edit its Markdown source; blur to render.
   ...
   editor has no slash menu, so this is a Notely differentiator. */
...
export default function BlockEditor({
  ...
  function convert(id: string, type: BlockType) {
    commit(
      blocks.map((b) =>
        b.id === id
          ? { ...b, type, text: type === "divider" ? "" : b.text.replace(/^\//, ""), rows: type === "table" ? [["", "", ""], ["", "", ""]] : b.rows }
          : b,
      ),
    );
    setSlash(null);
  ...
  function insertAfter(id: string, type: BlockType = "paragraph") {
    const nb: Block = type === "table" 
      ? { id: uuid(), type, text: "", rows: [["", "", ""], ["", "", ""]] }
      : { id: uuid(), type, text: "" };
    ...
    case "table":
      return (
        <div className="overflow-x-auto">
          <table
            onClick={onEdit}
            className="w-full cursor-text border-collapse overflow-hidden rounded-lg text-sm"
          >
            <tbody>
