...
export default function NoteView() {
  ...
  useEffect(() => {
    ...
    repo.getNote(id).then((n) => {
      ...
      n.lastOpenedAt = now();
      repo.putNote(n);
      sessionStorage.setItem("lastNoteId", n.id);
      setNote(n);
    });
    return () => {
      alive = false;
  ...
  useEffect(() => {
    if (missing) navigate("/app", { replace: true });
  }, [missing, navigate]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="border border-edge bg-panel rounded-xl p-4 text-center text-sm text-ink-dim">
          <p className="font-semibold text-ink mb-2">Automatic versions</p>
          <p>Notely saves a snapshot each time you stop typing. You can view and restore previous versions here. This feature will be available in a future update.</p>
        </div>
