...
const DATA_FOLDER =
  typeof navigator !== "undefined" && navigator.platform.startsWith("Win")
    ? "%APPDATA%\\Notely"
    : "~/Library/Application Support/Notely";
          ...
          <div className="rounded-card border border-edge bg-card p-6 shadow-soft">
            <div className="flex items-start justify-between">
              ...
              <div className="flex rounded-full border border-edge bg-panel p-1">
                {(["local", "cloud"] as EngineMode[]).map((m) => (
                  <button
                    key={m}
                    onClick={() => setMode(m)}
                    className={`rounded-full px-4 py-1.5 text-sm font-semibold ${
                      prefs.mode === m ? "bg-accent text-white" : "text-ink-faint hover:text-ink"
                    }`}
                  >
                    {m === "local" ? "Local" : "Cloud"}
                  </button>
            ...
            <div className="mt-4 flex items-center gap-3">
              <button
                onClick={async () => {
                  ...
                  downloadText("notely-notes.md", all, "text/markdown");
...
function LanguageField({
  ...
  const lang = LANGUAGES.find((l) => l.code === value)?.name ?? value;
  return (
    <div className="flex items-center justify-between rounded-xl border border-edge bg-panel px-4 py-3">
      <div className="min-w-0">
        ...
        <p className="truncate text-sm font-semibold">{lang}</p>
      </div>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="ml-4 shrink-0 rounded-xl border border-edge bg-card px-3 py-1.5 text-sm font-semibold outline-none focus:border-accent"
        aria-label={label}
      >
        {LANGUAGES.map((l) => (
