/* Utility functions for YouTube ingestion */
...
export async function extractCaptions(id: string, opts: { signal?: AbortSignal }): Promise<string | null> {
...
export async function fetchTitle(id: string): Promise<string | undefined> {
  try {
...
async function ingestYoutubeDesktop(): Promise<IngestResult> {
  // This would invoke the Tauri command
  // For now, we'll throw an error indicating this needs desktop app
  throw new Error(
...
async function ingestViaLocalServer(url: string): Promise<IngestResult | null> {
  ...
  try {
    ...
    return null; // SPA fallback answered "���������������� no helper on this host
  }
...
export async function ingestYoutube(url: string): Promise<IngestResult> {
  ...
  try {
    transcript = await extractCaptions(id, {});
  } catch {
    transcript = null;
  }
...
/* IngestResult type - importing from index would cause circular dependency */
