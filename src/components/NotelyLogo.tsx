export function NotelyLogo({ className = "h-8 w-8" }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 40 40"
      className={`logo-enter ${className}`}
      aria-hidden
    >
      <defs>
        <linearGradient id="notely-g" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="var(--color-sunrise)" />
          <stop offset="100%" stopColor="var(--color-butter)" />
        </linearGradient>
      </defs>
      <rect x="6" y="4" width="26" height="32" rx="6" fill="url(#notely-g)" />
      <rect x="10" y="4" width="2" height="32" fill="var(--color-ink)" opacity="0.25" />
      <path
